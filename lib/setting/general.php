	...
	class NgfbAdminGeneral extends NgfbAdmin {
		...
		protected function get_rows( $id ) {
			...
			switch ( $id ) {
				...
				case 'media' :
					...
					If the original image dimensions are less than the dimensions entered here, then the full-size image will be used instead.' ).
					...
					'Cropped '.$this->p->admin->form->get_checkbox( 'og_img_crop' ).'&nbsp;'.
					 ...
					 $this->p->admin->form->get_checkbox( 'og_img_resize' ).
					 '</td>';
					...
					'<td>'.$this->p->admin->form->get_input( 'og_def_img_id', 'short' ).' in the '.
					$this->p->admin->form->get_select( 'og_def_img_id_pre', $id_pre, 'medium' ).'</td>';
					...
					'<td>'.$this->p->admin->form->get_checkbox( 'og_def_img_on_search' ).'</td>';
					...
					'<td>'.$this->p->admin->form->get_checkbox( 'og_vid_https' ).'</td>';
				...
				case 'general' :
					...
					'<td>'.$this->p->admin->form->get_input( 'og_site_name', 
					...
					'<td>'.$this->p->admin->form->get_input( 'og_site_description', 
					...
					'<td>'.$this->p->admin->form->get_checkbox( 'og_desc_strip' ).'</td>';
				...
				case 'facebook' :
					...
					'<td>'.$this->p->admin->form->get_input( 'fb_admins' ).'</td>';
					...
					'<td>'.$this->p->admin->form->get_input( 'fb_app_id' ).'</td>';
					...
					'<td>'.$this->p->admin->form->get_select( 'fb_lang', 
				...
				case 'google' :
					...
					Check this option if you would like to force the <em>Default Author</em> on index webpages 
					(homepage, archives, categories, author, etc.).' ).
					'<td>'.$this->p->admin->form->get_checkbox( 'link_def_author_on_index' ).'</td>';

					$ret[] = $this->p->util->th( 'Default Author on Search Results', null, null, '
					Check this option if you would like to force the <em>Default Author</em> on search result webpages as well.' ).
					'<td>'.$this->p->admin->form->get_checkbox( 'link_def_author_on_search' ).'</td>';
					...
					'<td>'.$this->p->admin->form->get_input( 'link_publisher_url', 'wide' ).'</td>';
		...
		protected function get_rows_twitter() {
			return array(
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_site' ).
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_sum_size' ).
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_large_size' ).
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_photo_size' ).
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_gal_size' ).
				...
				<a href="https://dev.twitter.com/docs/cards/types/gallery-card" target="_blank">Gallery Card</a>.' ).
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_gal_min' ).
				...
				'<td class="blank">'.$this->p->admin->form->get_hidden( 'tc_prod_size' ).
				...
				'<td class="blank">'.
				$this->p->admin->form->get_hidden( 'tc_prod_def_l2' ).'Label: '.$this->p->options['tc_prod_def_l2'].' '.
				$this->p->admin->form->get_hidden( 'tc_prod_def_d2' ).'Value:'.$this->p->options['tc_prod_def_d2'].
				'</td>',
