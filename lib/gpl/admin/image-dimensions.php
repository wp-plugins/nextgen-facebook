...
if ( ! class_exists( 'NgfbGplAdminImagedimensions' ) ) {

	class NgfbGplAdminImagedimensions {
		...
		public function filter_image_dimensions_general_rows( $rows, $form ) {
			...
			$rows[] = $this->p->util->get_th( 'Twitter <em>Photo</em> Card', null, 'tc_photo_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'tc_photo' ).'</td>';

			$rows[] = $this->p->util->get_th( 'Twitter <em>Gallery</em> Card', null, 'tc_gal_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'tc_gal' ).'</td>';

			$rows[] = $this->p->util->get_th( 'Twitter <em>Product</em> Card', null, 'tc_prod_dimensions' ).
