...
if ( ! class_exists( 'NgfbAdminGeneral' ) ) {

	class NgfbAdminGeneral {
		...
		public function filter_og_webpage_rows( $rows, $form ) {
			...
			if ( $this->p->options['plugin_display'] == 'all' ) {
				...
				'<td class="blank">'.$form->get_fake_checkbox( 'og_page_title_tag' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Add Page Ancestor Tags', null, 'og_page_parent_tags' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_page_parent_tags' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Content Begins at First Paragraph', null, 'og_desc_strip' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_desc_strip' ).'</td>';
			}
		...
		public function filter_og_videos_rows( $rows, $form ) {
			...
			if ( $this->p->options['plugin_display'] == 'all' ) {
				...
				'<td class="blank">'.$form->get_fake_checkbox( 'og_def_vid_on_index' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Force Default Video on Author Index', null, 'og_def_vid_on_author' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_def_vid_on_author' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Force Default Video on Search Results', null, 'og_def_vid_on_search' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_def_vid_on_search' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Use HTTPS for Video API Calls', null, 'og_vid_https' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_vid_https' ).'</td>';
			}
			return $rows;
		}
		...
		public function filter_og_author_rows( $rows, $form ) {
			...
			if ( $this->p->options['plugin_display'] == 'all' ) {
				...
				'<td class="blank">'.$form->get_fake_checkbox( 'og_author_fallback' ).'</td>';
				...
				'<td class="blank">'.$form->get_fake_checkbox( 'og_def_author_on_index' ).' defines index webpages as articles</td>';

				$rows[] = $this->p->util->th( 'Default Author on Search Results', null, 'og_def_author_on_search' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'og_def_author_on_search' ).' defines search webpages as articles</td>';
			}

			$rows[] = $this->p->util->th( 'Gravatar Images for Authors', null, 'og_author_gravatar' ).
			'<td class="blank">'.$form->get_fake_checkbox( 'plugin_gravatar_api' ).'</td>';
		...
		public function filter_pub_google_rows( $rows, $form ) {
			...
			if ( $this->p->options['plugin_display'] == 'all' ) {
				...
				'<td class="blank">'.$form->get_fake_checkbox( 'seo_def_author_on_index' ).'</td>';
		
				$rows[] = $this->p->util->th( 'Default Author on Search Results', null, 'google_def_author_on_search' ).
				'<td class="blank">'.$form->get_fake_checkbox( 'seo_def_author_on_search' ).'</td>';
			}
