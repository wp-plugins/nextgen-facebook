...
if ( ! class_exists( 'NgfbGplAdminGeneral' ) ) {
	...
	class NgfbGplAdminGeneral {
		...
		public function filter_pub_twitter_rows( $rows, $form ) {
			...
			$rows[] = '<tr class="hide_in_basic">'.
			$this->p->util->get_th( 'Enable Twitter Card Pro Module', 'highlight', 'tc_enable' ).
			'<td class="blank"><input type="checkbox" disabled="disabled" /></td>';
			...
			$rows[] = $this->p->util->get_th( '<em>Photo</em> Card Image Dimensions', 'highlight', 'tc_photo_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'tc_photo' ).'</td>';

			$rows[] = '<tr class="hide_in_basic">'.
			$this->p->util->get_th( '<em>Gallery</em> Card Minimum Images', null, 'tc_gal_minimum' ).
			'<td class="blank">'.$this->p->options['tc_gal_min'].'</td>';
	
			$rows[] = $this->p->util->get_th( '<em>Gallery</em> Card Image Dimensions', null, 'tc_gal_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'tc_gal' ).'</td>';

			$rows[] = $this->p->util->get_th( '<em>Product</em> Card Image Dimensions', null, 'tc_prod_dimensions' ).
			'<td class="blank">'.$form->get_image_dimensions_text( 'tc_prod' ).'</td>';

			$rows[] = ( $this->p->is_avail['ecom']['*'] ? '' : '<tr class="hide_in_basic">' ).
			$this->p->util->get_th( '<em>Product</em> Card Maximum Labels', null, 'tc_prod_labels' ).
			'<td class="blank">'.$this->p->options['tc_prod_labels'].'</td>';

			$rows[] = ( $this->p->is_avail['ecom']['*'] ? '' : '<tr class="hide_in_basic">' ).
			$this->p->util->get_th( '<em>Product</em> Card Default 2nd Label', null, 'tc_prod_defaults' ).
