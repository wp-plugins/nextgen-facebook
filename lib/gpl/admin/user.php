...
if ( ! class_exists( 'NgfbGplAdminUser' ) ) {
	...
	class NgfbGplAdminUser {
		...
		public function filter_user_header_rows( $rows, $form, $post_info ) {
			...
			$rows[] = $this->p->util->th( 'Default / OG Description', 'medium', 'user-og_desc', $post_info ).
			'<td class="blank">'.$this->p->webpage->get_description( $this->p->options['og_desc_len'], 
				'...', false ).'</td>';
	
			$rows[] = $this->p->util->th( 'Search / SEO Description', 'medium', 'user-seo_desc', $post_info ).
			'<td class="blank">'.$this->p->webpage->get_description( $this->p->options['seo_desc_len'], 
				'...', true, true, false ).'</td>';	// no hashtags

			$rows[] = $this->p->util->th( 'G+ / Schema Description', 'medium', 'user-schema_desc', $post_info ).
