...
if ( ! class_exists( 'NgfbAdminAdvanced' ) ) {

	class NgfbAdminAdvanced {
		...
		public function filter_taglist_tags_rows( $rows, $form ) {
			...
			foreach ( $this->p->opt->get_defaults() as $opt => $val ) {
				if ( preg_match( '/^meta_([^_]+)_(.+)$/', $opt, $match ) ) {
					$cells[] = '<td class="taglist_checkbox blank">'.$form->get_fake_checkbox( $opt ).'</td>'.
						'<td clsss="taglist_type">'.$match[1].'</td>'.
						'<th class="taglist_name">'.$match[2].'</th>'."\n";
				}
			}
			$col_rows = array();
