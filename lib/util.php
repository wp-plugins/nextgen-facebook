...
if ( ! class_exists( 'NgfbUtil' ) && class_exists( 'SucomUtil' ) ) {
	...
	class NgfbUtil extends SucomUtil {
		...
		protected function add_actions() {
			...
			// add_plugin_image_sizes() is also called from the NgfbPost::set_head_meta_tags() method
		...
		public function clear_all_cache() {
			...
			if ( function_exists( 'w3tc_pgcache_flush' ) ) {	// w3 total cache
				w3tc_pgcache_flush();
				$this->p->notice->inf( __( 'W3 Total Cache has been cleared as well.', NGFB_TEXTDOM ), true );
			}
			if ( function_exists( 'wp_cache_clear_cache' ) ) {	// wp super cache
				wp_cache_clear_cache();
				$this->p->notice->inf( __( 'WP Super Cache has been cleared as well.', NGFB_TEXTDOM ), true );
			}
			if ( isset( $GLOBALS['zencache'] ) ) {			// zencache
				$GLOBALS['zencache']->wipe_cache();
				$this->p->notice->inf( __( 'ZenCache has been cleared as well.', NGFB_TEXTDOM ), true );
			}
		}
