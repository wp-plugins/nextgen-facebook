...
class ngfb_check_for_updates {
	...
	public function __construct($json_url, $file_path, $slug = '', $time_period = 12, $update_info_option = '', &$logger = '') {
	...
	public function install_hooks() {
		...
		if ($this->time_period > 0) {
			...
			if ( ! empty( $schedule ) && $schedule !== $this->sched_name ) {
				$this->logger->log('changing '.$this->cron_hook.' schedule from '.$schedule.' to '.$this->sched_name);
