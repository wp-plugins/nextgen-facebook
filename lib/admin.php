...
if ( ! class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbAdmin {
		...
		public function add_plugin_action_links( $links, $file ) {
			...
			if ( $file == NGFB_PLUGINBASE ) {
				...
				if ( $this->p->is_avail['aop'] ) {
					...
					array_push( $links, '<a href="'.$url['wp_support'].'">'.__( 'Forum', NGFB_TEXTDOM ).'</a>' );
					array_push( $links, '<a href="'.$url['purchase'].'">'.__( 'Purchase Pro', NGFB_TEXTDOM ).'</a>' );
				}
		...
		public function show_metabox_info() {
			...
			echo '<p><a href="'.$changelog_url.'" target="_blank">'.__( 'See the Changelog for more details...', NGFB_TEXTDOM ).'</a></p>';
			echo '</td></tr>';
			echo '</table>';
		}
