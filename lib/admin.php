...
if ( ! class_exists( 'NgfbAdmin' ) ) {
	...
	class NgfbAdmin {
		...
		protected function get_submit_buttons( $submit_text = '', $class = 'submit-buttons' ) {
			...
			// if ngfb is licensed, and the update manager is active, then allow update checks
			if ( ! empty( $this->p->options['plugin_'.$this->p->cf['lca'].'_tid'] ) && $this->p->is_avail['util']['um'] )
				$action_buttons .= $this->form->get_button( __( 'Check for Pro Update(s)', NGFB_TEXTDOM ), 
