	...
	class NgfbAdmin {
		...
		public function sanitize_options( $opts ) {
			...
			$this->p->notice->inf( __( 'Plugin settings have been updated.', NGFB_TEXTDOM ).' '.
				sprintf( __( 'Wait %d seconds for cache objects to expire (default) or use the \'Clear All Cache(s)\' button.', NGFB_TEXTDOM ), $this->p->options['plugin_object_cache_exp'] ), true );
			return $opts;
		}
				...
				elseif ( ! wp_verify_nonce( $_GET[ NGFB_NONCE ], $this->get_nonce() ) )
					...
					switch ( $_GET['action'] ) {
						...
						case 'clear_all_cache': 
							...
							$this->p->notice->inf( __( 'Cached files, WP object cache, transient cache, and additional caches like W3TC, Super Cache, and Quick Cache have been cleared.', NGFB_TEXTDOM ) );
							break;
		...
		protected function show_form_content() {
			if ( ! empty( $this->p->cf['*']['lib']['submenu'][$this->menu_id] ) ) {
				...
				echo '<form name="'.$this->p->cf['lca'].'" 
					id="'.$this->p->cf['lca'].'_settings_form" 
					method="post" action="edit.php?action='.NGFB_SITE_OPTIONS_NAME.'">';
