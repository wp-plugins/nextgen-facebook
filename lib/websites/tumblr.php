	...
	class ngfbAdminTumblr extends ngfbAdmin {
		...
		public function get_rows() {
			...
			return array(
				...
				'<th>Preferred Order</th><td>' . $this->select( 'tumblr_order', range( 1, count( $ngfb->social_options_prefix ) ), 'short' ) . '</td>',
...
if ( ! class_exists( 'ngfbSocialTumblr' ) && class_exists( 'ngfbSocial' ) ) {
	...
	class ngfbSocialTumblr extends ngfbSocial {
		...
		public function get_html( $atts = array() ) {
			...
			if ( ! empty( $atts['photo'] ) ) {
				$query .= 'photo?source='. urlencode( $ngfb->cdn_linker_rewrite( $atts['photo'] ) );
				$query .= '&amp;clickthru=' . urlencode( $atts['url'] );
				$query .= '&amp;caption=' . urlencode( $ngfb->str_decode( $atts['caption'] ) );
				...
				$query .= '&amp;source=' . urlencode( $ngfb->str_decode( $atts['title'] ) );
			} elseif ( ! empty( $atts['url'] ) ) {
				$query .= 'link?url=' . urlencode( $atts['url'] );
				$query .= '&amp;name=' . urlencode( $ngfb->str_decode( $atts['title'] ) );
				$query .= '&amp;description=' . urlencode( $ngfb->str_decode( $atts['description'] ) );
			}
