	...
	class ngfbCache {
		...
		public function get( $url, $want_this = 'url', $cache_name = 'file', $expire_secs = false, $curl_userpwd = '' ) {
			...
			if ( $want_this == 'raw' ) {
				...
				if ( ! empty( $cache_data ) ) {
					$this->debug->log( 'cache_data is present - returning '.strlen( $cache_data ).' chars' );
					return $cache_data;
				}
				...
				if ( file_exists( $cache_file ) && filemtime( $cache_file ) > time() - $file_expire ) {
					$this->debug->log( 'cache_file is current - returning cache url "'.$cache_url.'"' );
					return $cache_url;
				} else $this->debug->log( 'cache_file is too old or doesn\'t exist - fetching a new copy' );
			}
			...
			if ( ! empty( $this->ignore_urls ) && array_key_exists( $get_url, $this->ignore_urls ) ) {
				...
				if ( $time_remaining > 0 ) {
					$this->debug->log( 'ignoring URL '.$get_url.' for another '.$time_remaining.' second(s). ' );
					return $want_this == 'url' ? $url : '';
				} else {
			...
			if( ini_get('safe_mode') || ini_get('open_basedir') )
				$this->debug->log( 'PHP safe_mode or open_basedir defined, cannot use CURLOPT_FOLLOWLOCATION' );
			else {
			...
			$this->debug->log( 'curl: fetching cache_data from '.$get_url );
			$cache_data = curl_exec( $ch );
			$http_code = curl_getinfo( $ch, CURLINFO_HTTP_CODE );
			curl_close( $ch );

			$this->debug->log( 'curl: http return code = '.$http_code );
			if ( $http_code == 200 ) {
				if ( empty( $cache_data ) )
					$this->debug->log( 'cache_data returned from "'.$get_url.'" is empty' );
				elseif ( $this->save_cache_data( $cache_salt, $cache_data, $cache_name, $url_ext, $expire_secs ) == true ) {
					$this->debug->log( 'cache_data sucessfully saved' );
				...
				$this->debug->log( 'error connecting to URL '.$get_url.' for caching. ' );
				$this->debug->log( 'ignoring requests to cache this URL for '.$this->ignore_time.' second(s)' );
			...
			switch ( $cache_name ) {
				case 'wp_cache' :
					if ( defined( 'NGFB_OBJECT_CACHE_DISABLE' ) && NGFB_OBJECT_CACHE_DISABLE )
						$this->debug->log( 'object cache is disabled' );
					else {
				...
				case 'transient' :
					if ( defined( 'NGFB_TRANSIENT_CACHE_DISABLE' ) && NGFB_TRANSIENT_CACHE_DISABLE )
						$this->debug->log( 'transient cache is disabled' );
					else {
						$cache_type = 'object cache';
						$cache_id = $this->p->cf['lca']. '_'.md5( $cache_salt );	// add a prefix to the object cache id
						$this->debug->log( $cache_type.': cache_data '.$cache_name.' id salt "'.$cache_salt.'"' );
				...
				case 'file' :
					...
					if ( ! file_exists( $cache_file ) )
						$this->debug->log( $cache_file.' does not exist yet.' );
					elseif ( ! is_readable( $cache_file ) )
						$this->p->notices->err( '<u>'.$cache_file.'</u> is not readable.' );
					elseif ( $this->p->is_avail['aop'] !== true && ! is_admin() )
						$this->debug->log( 'file cache disabled: must be pro or admin.' );
					elseif ( filemtime( $cache_file ) < time() - $file_expire )
						$this->debug->log( $cache_file.' is expired (file expiration = '.$file_expire.').' );
					elseif ( ! $fh = @fopen( $cache_file, 'rb' ) )
						...
						fclose( $fh );
						if ( ! empty( $cache_data ) )
							$this->debug->log( $cache_type.': cache_data retrieved from "'.$cache_file.'"' );
					}
					break;
				default :
					$this->debug->log( 'unknown cache name "'.$cache_name.'"' );
					break;
			}
			...
			switch ( $cache_name ) {
				...
				case 'file' :
					...
					if ( ! is_writable( $this->base_dir ) )
						$this->p->notices->err( '<u>'.$this->base_dir.'</u> is not writable.' );
					elseif ( $this->p->is_avail['aop'] == true || is_admin() ) {
						if ( ! $fh = @fopen( $cache_file, 'wb' ) )
							$this->p->notices->err( 'Failed to open <u>'.$cache_file.'</u> for writing.' );
						else {
							if ( fwrite( $fh, $cache_data ) ) {
								$this->debug->log( $cache_type.': cache_data saved to "'.$cache_file.'"' );
								$ret_status = true;	// success
							}
							fclose( $fh );
						}
