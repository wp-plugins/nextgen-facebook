...
**A.** NextGEN Facebook OG uses the "ngfb-buttons" CSS class name to wrap all social buttons, and each button has it's own individual class name as well.  Here's an example of the CSS I'ved used on <a href="http://surniaulula.com/">Surnia Ulula</a>:
...
The "NGFB Social Buttons" widget adds an extra CSS class name you can use to create a different layout. For example, here are different styles for social buttons in a widget and added to the content.
...
This assumes you are calling the function from within a loop, a post or page (the $post->ID must be defined). Social button names for the array can be 'facebook', 'gplus', 'twitter', 'linkedin', 'linkedin', 'pinterest' and 'tumblr'.

You can also use the function <em>outside</em> of a loop, post, or page, but you will have to provide additional information to the function. Since the $post->ID is not available to get the permalink, at a minimum you will have to provide the webpage URL. Here's an example from a custom NextGEN Gallery template: 
